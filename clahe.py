# -*- coding: utf-8 -*-
"""
Created on Tue May 26 14:24:19 2020
...
def clahe(img, clipLimit, nrBins, nrX, nrY):
    """
    ...
    if clipLimit > 0:
        clipLimit = max(1, clipLimit * xsz * ysz / 256)
    ...
    for i in range(nrX):
        for j in range(nrY):
            ...
            bin_ = img[i * xsz:(i + 1) * xsz, j * ysz:(j + 1) * ysz].astype(int)
            for i1 in range(xsz):
                for j1 in range(ysz):
    ...
    if clipLimit > 0:
        for i in range(nrX):
            for j in range(nrY):
                ...
                if nrExcess > 0:
                    stepSz = max(1, np.floor(1 + nrExcess / 256))
                    for nr in range(256):
                        nrExcess -= stepSz
                        hist[i, j, nr] += stepSz
                        if nrExcess < 1:
...
if __name__ == '__main__':
    clip_size = 2
    gray_image = image_read('../image/lacnormal.png')
